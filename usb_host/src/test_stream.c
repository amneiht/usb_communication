...
void test_stream(usbdc_stream *ls) {
	...
	while (1) {
		i++;
		mess.length = (i % 20) + 20;
		int ret = usbdc_stream_read_package(ls, 0, mess2.line_buf, USB_MAX);
		if (ret > 0)
			printf("mess is %.*s\nleng is :%d\n",ret , mess2.line_buf, ret);
		usbdc_stream_write(ls, 0, mess.line_buf , mess.length);
		usleep(200 * 1000);
	}
}
